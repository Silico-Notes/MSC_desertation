...
| **Archival variance** | Documentation variations or uncertainties that cannot be definitively resolved without speculation |

This classification prevents historical claims from being presented as independently reproduced results, maintaining strict scientific provenance.
...
* **Tumor Accession:** The thesis Methods section identifies PRJNA521318, whereas the Results section references PRJNA636799. This provenance variance is explicitly preserved.
* **Tumor Cohort:** 11 total samples (described as 8 SCC + 3 adenocarcinoma).
* **Control Cohort:** 10 selected healthy samples.

> **Sample Documentation Variance:** The historical public universe spans 12 tumor accessions (SRR8561801–SRR8561812) and 11 control accessions (SRR7946354–SRR7946364). However, the recovered computational analysis matrix contains exactly 21 samples (SRR8561802–SRR8561812 for tumors, SRR7946355–SRR7946364 for controls). SRR8561801 and SRR7946354 remain as archival variances and are not presented here as intentionally excluded.
...
> **Note on Workflow Tooling:** While the original study design evaluated multiple differential expression engines (DESeq2, edgeR, Cuffdiff), the recovered computational evidence quantifies the final dataset specifically using DESeq2.
> 
> 
...
> **Archival Variance Note:**
> * **Thesis-reported result:** 523 DEGs (314 upregulated + 209 downregulated).
> * **Recovered result:** 521 protein_coding genes.
> 
> 
> The difference of two genes remains an archival variance. This repository embraces this distinction to demonstrate provenance-aware reproducibility rather than hiding the inconsistency.
...
> **Hub-Gene Archival Variance:** The historical sources contain multiple levels of reporting: the abstract/conclusion reports 317 hub genes, the detailed Results section describes 100 recurring genes/candidates, and it also details additional smaller phase-specific candidate sets. Surviving evidence does not reconcile these counts, representing an archival reporting variance.
> 
> 
...
**Translational Potential:** The top hub genes (e.g., CDK1, MMP9, FN1) identified in this transcriptomic network offer a foundational starting point for structure-based drug discovery. In future iterations, these computational candidates could be advanced through targeted molecular docking and molecular dynamics simulations to evaluate their structural viability as therapeutic targets.

### Key Results at a Glance

| Metric | Value |
| --- | --- |
| **Total DEGs** | 14,160 (314 Upregulated / 209 Downregulated) |
| **Final Candidates** | 523 (Thesis-Reported) / 521 (Recovered) |
| **Significant KEGG Pathways** | 70 (10 Highlighted) |
| **STRING Expected Edges** | 282 (Enrichment < 1e-16) |
...
* `HISTORICAL_WORKFLOW.md` — Detailed description of the original computational workflow.
* `FORENSIC_INVENTORY.md` — Inventory of recovered project evidence.
* `FORENSIC_SUMMARY.md` — Summary of the computational recovery.
* `RECONSTRUCTION_LOG.md` — Records what could and could not be reconstructed.
* `FILE_MANIFEST.tsv` — Structured inventory of relevant project files.
* `FUTURE_WORK.md` — Proposed modernization and reproducibility work.
...
4. **Archival variance:** Contradictions or documentation variations that cannot be definitively resolved.
...
* Archival variances regarding accessions (PRJNA521318 vs PRJNA636799), samples (21 vs 23), and DEGs (521 vs 523) remain unresolved.
...
**FUTURE WORK:** Proposed plans for modernization are separately documented in `FUTURE_WORK.md`. This includes:

* Porting the legacy Galaxy web workflow into a modern, reproducible, and containerized **Nextflow** or **Snakemake** pipeline.
* Implementing robust database solutions, such as **PostgreSQL**, to manage complex genomic metadata.
* Translating these transcriptomic findings into actionable therapeutic insights via **structure-based drug discovery** pipelines.
...
This repository represents an MSc Bioinformatics  project reorganized into a transparent portfolio format. It intentionally preserves historical results and clearly identifies archival variances instead of retroactively modifying the original conclusions.
